...
$stdout = Join-Path $env:TEMP ("seo-vital-monitor-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("seo-vital-monitor-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5372"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/vitals-lane"; File = "02-vitals-lane-proof.png"; Size = "1600,1380" },
        @{ Url = "http://127.0.0.1:$port/route-regressions"; File = "03-route-regressions-proof.png"; Size = "1600,1260" },
